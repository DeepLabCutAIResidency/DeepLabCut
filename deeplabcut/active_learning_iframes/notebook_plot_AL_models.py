...
parent_dir_path_list = ['Horse10_AL_unif_OH','Horse10_AL_uncert_OH','Horse10_AL_infl_OH']
...
for (model_prefix,parent_dir) in zip(model_prefix_list,
    ...
    list_models_dirs = [x for x in os.listdir(parent_dir) if x.startswith(model_prefix) and not x.h('pkl')]  
    list_models_dirs.sort()
    # get normalised pixel error for the models in this strategy
    path_to_pickle_px_error = os.path.join(parent_dir,
                                           model_prefix+'_px_error.pkl')
    with open(path_to_pickle_px_error,'rb') as file:
        [_, dict_norm_px_error_total_per_model] = pickle.load(file)
