'''
Aux fns to generate a train/test splits in horse dataset
...
def split_test_set_dicts(map_shuffle_id_to_test_dirs_w_nfiles):
    '''
    Split test set into AL and OOD sets, solving the min sum partition problem

    Given the map with test directories to number of files, split in two groups such that the difference 
    between total number of frames in each group is minimal
...
def convert_dir_list_to_dict_w_nfiles(list_dirs,
                                      ...
                                      files_ext_w_dot): #'.png'
    '''
    Convert list of subdirectories in parent directory to dictionary with
        keys = fullpath to subdirectories
        values = number of files in subdirectory with extension files_ext_w_dot
    '''                                  
    ...
    for d in list_dirs:
        num_files_in_d = len([f for f in os.listdir(os.path.join(parent_dir_path,d)) if f.endswith(files_ext_w_dot)])
...
def compute_dicts_w_nfiles_in_train_test_sets_per_shuffle(parent_dir_path,
    ...
    - dict mapping horse directories full paths to their number of files (map_horses_dirs_to_nfiles)
    - dict mapping each shuffle to a dict with the train directories full paths mapped to 
        their number of files (map_shuffle_id_to_train_dirs_w_nfiles)
    - dict mapping each shuffle to a dict with the rain directories full paths mapped to 
        their number of files (map_shuffle_id_to_test_dirs_w_nfiles)
    
    The test directories here include active learning set and OOD set
    """                              
    ########################################
...
def compute_idcs_from_list_dirs(list_dirs,
                                df):
    '''
    Given a list of directories, compute 
    '''                            
